...
function initialize() {
    ...
    $.getJSON('https://elenaran.cartodb.com/api/v2/sql/?q=SELECT title, class FROM schools ORDER BY title', function(data) {
...
function findCity() {
    ...
    if (which1 != "" && which2 != "") {
        ...
        $.getJSON('https://elenaran.cartodb.com/api/v2/sql/?q=' + sql1, function(data) {
            ...
            sql2 += " * FROM stadiums_usage where capacity>=" + parseInt($('#slider-range [class*=ui-slider-pip-selected] .ui-slider-label')[0].innerHTML.replace(/,/g, '')) + " AND capacity<=" + parseInt($('#slider-range [class*=ui-slider-pip-selected] .ui-slider-label')[1].innerHTML.replace(/,/g, ''));
